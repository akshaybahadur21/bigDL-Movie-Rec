...
Movie recommendations have been cached
""")
...
if st.button('Press to generate insights from data'):
    ...
    st.markdown('**1.2. Number of unique users**')
    st.info(len(df))

    st.markdown('**1.3. Number of unique Movies**')
    movie_list = []
    ...
    st.markdown('**1.4. Sample Movie Names**')
    st.info((movie_list)[0:10])

    st.markdown('**1.5. Number of unique Genres**')
    set_genre = set()
    ...
    st.markdown('**1.6. Sample Genre Names**')
